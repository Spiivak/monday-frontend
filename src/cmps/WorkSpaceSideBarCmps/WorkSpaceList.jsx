        ...
        <div key={board._id}>
          <div className="workspace-item flex align-center space-between">
            <div className="link flex space-between">
              <NavLink to={`/workspace/${board._id}`} className="flex gap8 align-center hoverable space-between">
                <div className="nav-link-content flex align-center gap8">
                  <BoardIcon />
                  {board.title}
                </div>
            <ContextBtn type='board' onDeleteBoard={()=>onRemoveBoard(board._id)}/>
              </NavLink>
            </div>
          </div>
        </div>
