...
export function BuildingBlocksTabs({ activeTab, onTabClick }) {
  return (
    <div className="building-blocks-tabs flex justify-center align-center">
      {tabsData.map((tab) => (
      <NavLink >
        <div key={tab.id} className={`tab ${tab.id} flex column align-center ${activeTab === tab.id ? 'active' : ''}`} onClick={() => onTabClick(tab.id)}>
          <picture ><img src={tab.svg} alt={tab.label} /></picture>
          <span>{tab.label}</span>
        </div>
      </NavLink>
      )
